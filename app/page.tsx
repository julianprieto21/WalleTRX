...
export default async function HomePage() {
  ...
  if (!user) return;
  return (
    <main className="page pt-6 2xl:pt-12 pb-6 px-4 2xl:px-10">
      ...
      <div className="w-full h-full 2xl:h-auto flex flex-col 2xl:flex-row gap-0 2xl:gap-8">
        <Suspense
          fallback={
            <div className=" shrink-0 rounded-lg h-96 w-[620px] mt-4 2xl:mt-10 shadow-md bg-palette-300 px-6 py-5"></div>
          }
        >
          <MainCard user={user} />
        </Suspense>
        <Suspense
          fallback={
            <div className="hidden 2xl:block rounded-lg h-96 w-full mt-6 2xl:mt-10 shadow-md bg-palette-300 px-6 py-5"></div>
          }
        >
          <Last5transactions />
        </Suspense>
      </div>
      <div className="hidden 2xl:block w-full h-72 bg-palette-300 rounded-lg shadow-md mt-8">
        <Suspense
          fallback={
